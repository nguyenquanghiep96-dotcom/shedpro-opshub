...
// Add state for confirmDrop
content = content.replace("const [selectedRoute, setSelectedRoute] = useState<RouteData | null>(null);", "const [selectedRoute, setSelectedRoute] = useState<RouteData | null>(null);\n  const [confirmDrop, setConfirmDrop] = useState<{routeId: string, date: Date} | null>(null);");

// Update onDrop to set state instead of updating directly
content = content.replace("onDrop={(e) => { const routeId = e.dataTransfer.getData('routeId'); if(routeId) updateRouteDate(routeId, dayObj.date); }}", "onDrop={(e) => { const routeId = e.dataTransfer.getData('routeId'); if(routeId) setConfirmDrop({routeId, date: dayObj.date}); }}");

// Add Confirm Modal at the end of return statement
const confirmModal = `
      {/* Confirm Drop Modal */}
      {confirmDrop && (
