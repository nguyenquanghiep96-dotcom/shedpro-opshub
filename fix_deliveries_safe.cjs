...
const bottomReplace = `        </div>
      </div>
    </div>
    </TransportationProvider>
  );
}`;
// We only want to replace the last occurrence, which is at the end of the file.
const lastIdx = del.lastIndexOf(bottomSearch);
if (lastIdx !== -1) {
    del = del.substring(0, lastIdx) + bottomReplace + del.substring(lastIdx + bottomSearch.length);
}
