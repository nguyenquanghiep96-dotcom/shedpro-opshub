...
const tabCode = extractFunction('Tab');
if (tabCode) {
  content = content.replace(tabCode, '');
  fs.writeFileSync(file, content);
  
  let headerNavContent = fs.readFileSync('src/components/layout/HeaderNav.tsx', 'utf8');
  // insert Tab before export
  headerNavContent = headerNavContent.replace('export {', tabCode + '\n\nexport {');
  fs.writeFileSync('src/components/layout/HeaderNav.tsx', headerNavContent);
  console.log("Moved Tab to HeaderNav.tsx");
} else {
